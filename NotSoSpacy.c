...
struct currentTime {
    //struct used by fetchTime() to store the current time
    int hour;
    int minute;
} curTime;
...
//Responsible for handling the increement and decreement button logic for hours
void alarmMinButton(GtkButton *button, gpointer user_data);

//Responsible for handling the increement and decreement button logic for minutes
void setAlarm();

int alarmCount = 0; //tracks how many alarms are there
struct alarms {
    //struct used to store all the alarms created
    int hour;
    int minute;
