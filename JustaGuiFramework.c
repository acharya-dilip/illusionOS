...
void screenWindowChild() {
    ...
    gtk_window_set_transient_for(GTK_WINDOW(windowChild),windowMainDesktop);
...
void widgetAttributeWidgets(GtkWidget *window, int i) {
    ...
    GtkAdjustment *defaultAdjustmentColumnSpan =
            gtk_adjustment_new(widget[i].grid.colSpan, 0.0, 1000.0, 1.0, 10.0, 0.0);
    GtkAdjustment *defaultAdjustmentMinHeight = gtk_adjustment_new(widget[i].minSize.height, 0.0, 1000.0, 1.0, 10.0,
                                                                   0.0);
    GtkAdjustment *defaultAdjustmentMinWidth = gtk_adjustment_new(widget[i].minSize.width, 0.0, 1000.0, 1.0, 10.0, 0.0);
    GtkAdjustment *defaultAdjustmentMarginStart = gtk_adjustment_new(widget[i].margin.start, 0.0, 1000.0, 1.0, 10.0,
                                                                     0.0);
    ...
    GtkAdjustment *defaultAdjustmentMarginBottom = gtk_adjustment_new(widget[i].margin.bottom, 0.0, 1000.0, 1.0, 10.0,
                                                                      0.0);
...
void registerWidget(GtkButton *button, gpointer user_data) {
    ...
    const char *widgetType = gtk_string_list_get_string(GTK_STRING_LIST(listWidgets),
                                                        gtk_drop_down_get_selected(GTK_DROP_DOWN(dropdownWidgets)));
...
void JustaGuiFramework() {
    ...
    gtk_widget_add_css_class(labelWidgets, "BigLabel");
...
void exportGuiData() {
    ...
    fprintf(file2,
            "S.N. \t Widget Name \t Widget Type \t Row \t Row Span \t Column \t Column Span \t Min Width \t Min Height \t Margin Top \t Margin Bottom \t Margin Start \t Margin End \n");
    for (int i = 0; i < widgetCount; i++) {
